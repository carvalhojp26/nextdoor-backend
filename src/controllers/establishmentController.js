...
const getEstablishmentsByNeighborhoodController = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({message: "Establishments fetched successfully", establishments: establishments});
  } catch (error) {
...
const getEstablishmentByIdController = async (req, res) => {
  ...
  try {
    const user = await userService.getUser(userId);
    const neighborhoodId = user.VizinhançaidVizinhança;
    const establishment = await establishmentService.getEstablishmentById(establishmentId, neighborhoodId);

    res.status(200).json({message: "Establishment fetched successfully", establishment: establishment,});
  } catch (error) {
    res.status(500).json({ error: "Internal server error" });
  }
};
...
const createEstablishmentController = async (req, res) => {
  ...
  try {
    ...
    const {telefoneEstabelecimento, emailEstabelecimento, nomeEstabelecimento, EnderecoidEndereco, VizinhancaidVizinhanca} = req.body;
    if (
...
const updateEstablishmentController = async (req, res) => {
  ...
  try {
    ...
    const establishment = await establishmentService.updateEstablishment(establishmentId, req.body);
    res.status(200).json({ message: "Establishment updated successfully", updatedRows: establishment });
  } catch (error) {
    res.status(500).json({ error: "Internal server error" });
  }
};
...
const deleteEstablishmentController = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Establishment deleted successfully", user: result });
  } catch (error) {
    res.status(500).json({ error: "Internal server error" });
  }
};
...
module.exports = {getEstablishmentsByNeighborhoodController, getEstablishmentsController, getEstablishmentByIdController, createEstablishmentController, updateEstablishmentController, deleteEstablishmentController}